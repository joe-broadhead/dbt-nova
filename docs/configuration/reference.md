...
- `DBT_NOVA_AGENT_MODELLING_AUDIT_ENABLED` – emit deterministic `agent_modelling_findings` in `modelling_consistency_report` (`true`|`false`, default: `true`)
- `DBT_NOVA_AGENT_MODELLING_MAX_FINDINGS` – max `agent_modelling_findings` retained in `modelling_consistency_report` (default: `100`)
- `DBT_NOVA_AGENT_MODELLING_TOO_MANY_PARENTS_THRESHOLD` – direct-parent threshold for `agent_surface_too_many_parents` findings (default: `7`)
- `DBT_NOVA_AGENT_MODELLING_SOURCE_FANOUT_THRESHOLD` – reserved source-fanout threshold for future source-shape audit checks (default: `20`)
- `DBT_NOVA_AGENT_MODELLING_ENABLE_SQL_SHAPE_CHECKS` – opt in to future SQL-shape audit checks (`true`|`false`, default: `false`)
- `DBT_NOVA_AGENT_READINESS_MODELLING_MAX_BLOCKERS` – readiness threshold for blocker modelling findings (default: `0`)
- `DBT_NOVA_AGENT_READINESS_MODELLING_MAX_HIGH` – readiness threshold for high-severity modelling findings (default: `10`)
- `DBT_NOVA_AGENT_READINESS_MODELLING_MAX_BLOCKERS_REQUIRED` – make blocker-threshold readiness failures required instead of advisory (`true`|`false`, default: `true`)
- `DBT_NOVA_AGENT_READINESS_MODELLING_MAX_HIGH_REQUIRED` – make high-severity readiness threshold failures required instead of advisory (`true`|`false`, default: `false`)
