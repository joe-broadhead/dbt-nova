...
- DuckDB preflight checks for `preflight_catalog`, `preflight_schema`, and `preflight_relation` require a non-empty probe result; missing/inaccessible targets return `ok=false` in checks.
