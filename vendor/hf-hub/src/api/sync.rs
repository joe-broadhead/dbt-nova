...
pub enum ApiError {
    ...
    /// Redirect location cannot be resolved into an absolute URL
    #[error("Redirect location '{location}' is invalid for request '{request_url}'")]
    InvalidRedirectLocation {
        /// The original request URL that produced the redirect.
        request_url: String,
        /// The raw `Location` header value returned by the server.
        location: String,
    },
...
impl ApiBuilder {
    ...
    /// Overrides the Hub API endpoint.
...
fn resolve_redirect_location(request_url: &str, location: &str) -> Result<String, ApiError> {
    if let Ok(url) = Url::parse(location) {
        return Ok(url.into());
    }

    let base = Url::parse(request_url).map_err(|_| ApiError::InvalidRedirectLocation {
        request_url: request_url.to_string(),
        location: location.to_string(),
    })?;
    base.join(location)
        .map(Into::into)
        .map_err(|_| ApiError::InvalidRedirectLocation {
            request_url: request_url.to_string(),
            location: location.to_string(),
        })
}
...
impl Api {
    ...
    fn metadata(&self, url: &str) -> Result<Metadata, ApiError> {
        ...
        let response = if is_redirection {
            ...
            let redirect_url = resolve_redirect_location(url, location)?;
            self.client
                .get(&redirect_url)
                .set(RANGE, "bytes=0-0")
                .call()
