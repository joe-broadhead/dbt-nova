...
impl TextEmbedding {
    ...
    pub fn try_new(options: InitOptions) -> Result<Self> {
        ...
        let threads = threads.unwrap_or_else(|| available_parallelism().map_or(1, |value| value.get()));
        ...
        let model_file_reference = model_repo
            .get(&model_file_name)
            .map_err(|error| anyhow!("Failed to retrieve {}: {error}", model_file_name))?;
        ...
        if model_name == EmbeddingModel::MultilingualE5Large {
            model_repo
                .get("model.onnx_data")
                .map_err(|error| anyhow!("Failed to retrieve model.onnx_data: {error}"))?;
        }
